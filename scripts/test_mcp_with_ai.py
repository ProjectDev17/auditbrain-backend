...
def ask_llm(question):
    response = ollama.chat(
        ...
        messages=[{
            "role": "system",
            "content": (
                "You are an AI agent for an audit management system. "
                "Based on the user's question, respond with EXACTLY ONE intent.\n\n"
                "Available intents:\n"
                "- COUNT_IN_PROGRESS (for questions about in-progress audits)\n"
                "- COUNT_PENDING (for questions about pending audits)\n"
                "- COUNT_COMPLETED (for questions about completed audits)\n"
                "- SHOW_LAST_AUDITS (for listing recent audits)\n"
                "- SUMMARY (for questions asking about multiple statuses, totals, or comparisons)\n\n"
                "Rules:\n"
                "1. Respond with ONLY the intent name, nothing else\n"
                "2. If the question asks about multiple things, use SUMMARY\n"
                "3. If unclear, use SUMMARY"
            )
        }, {
            "role": "user",
            "content": question
...
def generate_response(question, summary):
    ...
    response = ollama.chat(
        ...
        messages=[{
            "role": "system",
            "content": (
                "You are a helpful assistant for an audit management system. "
                "Based on the user's question and the data provided, "
                "give a concise, friendly response in the same language as the user's question. "
                "Be specific and answer exactly what the user asked. "
                "If they ask for a sum or total, calculate it. "
